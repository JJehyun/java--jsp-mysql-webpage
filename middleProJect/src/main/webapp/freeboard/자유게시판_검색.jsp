...
int count = DAO.Freeboardcountsearch(search);  //게시글 총 갯수 확인 함수
         ...
         <div class="outer-container">
           <div style="display:flex;justify-content:space-between;">
             ...
             <table id="middletable">
                 ...
                 <%String query = "select 529freeboard.id,529freeboard.name,529freeboard.title,529freeboard.counter,DATE_FORMAT(time,'%y-%m-%d'),529userinfor.userID,529userinfor.Email,529userinfor.name,529userinfor.IMG from 529freeboard INNER JOIN 529userinfor ON 529freeboard.name = 529userinfor.userID where 529freeboard.title like ? order by 529freeboard.id desc limit ?,?";
                 stmt = con.prepareStatement(query);
                 stmt.setString(1,'%'+search+'%');
                 stmt.setInt(2,startpage);
                 stmt.setInt(3,onepagecnt);
...
<div style="border:0px solid black; width:100px;"></div>             
             ...
             <ul class="pagination">
             <%if(pass == 1){ %><li><a href="http://localhost:8080/middleProJect/freeboard/자유게시판.jsp" onclick="alert('처음 페이지 입니다.');" class="prev">< Prev</a></li><%}else{ %><li><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=pass-1%>&q=<%=search%>" class="prev">< Prev</a></li><%} %>
               <%for(int i=pass-1; i<pass+2; i++){%>
               <%if(pass == count){%><a style="color:black;padding-top: 13px;">마지막 페이지</a><%break;} %>
                <%if(i == 0){%><li class="pageNumber active"><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=1 %>&q=<%=search%>"><%=1 %></a></li><li class="pageNumber active"><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=2 %>&q=<%=search%>"><%=2 %></a></li><li class="pageNumber active"><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=3 %>&q=<%=search%>"><%=3 %></a></li><%break;} %>
               <li class="pageNumber active"><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=i %>"><%=i %></a></li><%} %>
               <%if(pass == count){ %><li><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=pass%>" onclick="alert('마지막 페이지 입니다.');" class="next">Next ></a></li><%}else {%><li><a href="http://localhost:8080/middleProJect/freeboard/자유게시판_검색.jsp?page=<%=pass+1%>&q=<%=search%>" class="next">Next ></a></li><%} %>
             </ul>
             </div>
<div class="wrap">
