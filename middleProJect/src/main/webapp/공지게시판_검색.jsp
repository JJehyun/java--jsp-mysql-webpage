...
DAO.Noticeorigin();
String sql = "SELECT COUNT(*) FROM board WHERE title LIKE ? LIMIT ?,?";
...
if(tempstart !=null){
   ...
   String query = "select * from 529notice where description like ? ORDER BY id desc limit ?,?";
...
<%} %>
             ...
             <div class="footer-tools" style="position:absolute; top:95%; width:100%">
               ...
               <div class="pages" style="position:absolute; right:100;">
                 <ul><li><button id="previous" onclick="location.href='http://localhost:8080/middleProJect/공지게시판_검색.jsp?page=<%=pass-1 %>&q=<%=search %>'">◀</button></li>
                 <%for(int i=pass-1; i<pass+2; i++){%>
                 <%if(i == 0){%><li><button onclick="location.href='http://localhost:8080/middleProJect/공지게시판_검색.jsp?page=<%=1 %>&q=<%=search %>'"><%=1 %></button></li><li><button onclick="location.href='http://localhost:8080/middleProJect/공지게시판_검색.jsp?page=<%=2 %>&q=<%=search %>'"><%=2 %></button></li><%break;}%>
                 <%if(pass == count){%><a>마지막 페이지</a><%break;} %>
                 <li><button onclick="location.href='http://localhost:8080/middleProJect/공지게시판_검색.jsp?page=<%=i %>&q=<%=search %>'"><%=i %></button></li>
                 <%} %>
                 <li><button id="next" onclick="location.href='http://localhost:8080/middleProJect/공지게시판_검색.jsp?page=<%=pass+1 %>&q=<%=search %>'">▶</button></li>
                 </ul>
               </div>
             </div>
           </table>
