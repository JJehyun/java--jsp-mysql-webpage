 ...
 <html>
  <head>
     <meta charset="utf-8">
        ...
        <link href="stylejs/detailpage.css" rel="stylesheet">
...
String username = DAO.Noticedetail(titlenumber);
String usernames = DAO.Noticedetailname(titlenumber);
String userage = DAO.Noticedetailage(titlenumber);
String useremail = DAO.Noticedetailemail(titlenumber);
String userimg = DAO.Noticedetailimg(titlenumber);
String usertitle = DAO.Noticetitle(titlenumber);
String usertime = DAO.Noticetime(titlenumber);
String usercounter = DAO.Noticecounter(titlenumber);
DAO.noticecounter(titlenumber);
String userdescription = DAO.Noticedetaildescription(titlenumber);
String userpoto = DAO.Noticedetailpoto(titlenumber);
...
<div style="display:flex;">
        <ul class="sidebar-menu">
           ...
           <div id="user_information_border">
             <span style="text-align:center;"><%=usernames%></span>
           <div id="user_information_border_under"></div>
            <table>
              <tr>
                <td rowspan="3" style="width:40%"><img src="http://localhost:8080/middleProJect/userPicture/<%=userimg %>"  alt="userimg" height="150" width="150" id="circleimg"></td>  <td style="text-align:left; height:20px;"><%=username%></td>  <td style="text-align:left"><%=userage %></td>
              </tr>
              <tr>
...
<table style="width:100%">
  ...
  <%String query = "select title,DATE_FORMAT(time,'%y-%m-%d'),id from 529notice ORDER BY id desc limit 0,4";
    stmt = con.prepareStatement(query);
	rs = stmt.executeQuery();
	while(rs.next()){%><tr><td id="tdwidthten"><span style="margin-right:15px;color:red">공지</span></td><td style="width:70%;"><a href="http://localhost:8080/middleProJect/공지게시판_상세페이지.jsp?id=<%=rs.getString(3)%>"><%=rs.getString(1) %></a></td><td style="width:20%"><%=rs.getString(2) %></td></tr><%}%>
...
<%if(userpoto == null){ %>
<center><%=userdescription %></center><%}else{%><center><img width="450px" height="300px" src="http://localhost:8080/middleProJect/notice/<%=userpoto %>"></center>
...
<center><div style="border:0px solid gray; width:73%; height:90px;">
  ...
  <div class="container_coment">
    <form action="http://localhost:8080/middleProJect/공지게시판_상세페이지_확인.jsp?id=<%=titlenumber %>" method="post">
    ...
    <div style="display:flex;">
      ...
      <input type="submit" href="http://localhost:8080/middleProJect/공지게시판_상세페이지_확인?.jsp" class="postsubmit" value="post"></input>
    </div>
      </div>
    </form>
  </div>
...
<%
String querythree = "select 529notice.id,529noticedetail.detailcontents,529noticedetail.time,529noticedetail.page,529noticedetail.username,529userinfor.IMG from 529notice inner join 529noticedetail on 529notice.id = 529noticedetail.page inner join 529userinfor ON 529noticedetail.username = 529userinfor.userID where 529notice.id= ?";
stmt = con.prepareStatement(querythree);
stmt.setString(1, titlenumber);
rs = stmt.executeQuery(); 
while(rs.next()){%>
<tr><td rowspan="2" style="margin-right:50px; width:5%;"><img width="50px;" height="50px;" src="http://localhost:8080/middleProJect/userPicture/<%=rs.getString(6)%>" /></td><td style="text-align:left;"><span style="margin-right:10px;"><%=rs.getString(5)%></span><span><%=rs.getString(3)%></span></td>    <td style="width:5%; text-align:right; color:red;"><span>X</span></td><td style="width:5%; text-align:center;"><span class="postsubmit">답글</span></td></tr>
